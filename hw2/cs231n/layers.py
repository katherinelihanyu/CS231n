...
def batchnorm_backward(dout, cache):
	...
	x, normalized, mean, var, gamma, eps = cache
	###########################################################################
	...
	N,_ = x.shape
	dxn = dout * gamma
	dvar = np.sum(dxn*(x-mean)*(-0.5)*(var+eps)**(-1.5),axis=0)
	dmean = np.sum(dxn*(-1/np.sqrt(var+eps)),axis=0) + dvar*np.sum(-2*(x-mean),axis=0)/N
...
def max_pool_forward_naive(x, pool_param):
	...
	for n in range(N):
		for ho in range(Ho):
			h1 = ho*stride
			h2 = pool_height + ho*stride
			for wo in range(Wo):
				w1 = wo*stride
				w2 = pool_width + wo*stride
				out[n,:,ho,wo] = np.amax(x[n,:,h1:h2,w1:w2], axis=(1,2))
	###########################################################################
	#                             END OF YOUR CODE                            #
	###########################################################################
	cache = (x, pool_param)
	...
	for n in range(N):
		for ho in range(Ho):
			...
			for wo in range(Wo):
				w1 = wo*stride
				w2 = pool_width + wo*stride
				maxarr = np.amax(x[n,:,h1:h2,w1:w2], axis=(1,2))
				for hh in range(pool_height):
					for ww in range(pool_width):
						for c in range(C):
							if x[n,c,ho*stride+hh,wo*stride+ww] == maxarr[c]:
								dx[n,c,ho*stride+hh,wo*stride+ww] += dout[n,c,ho,wo]
	###########################################################################
	#                             END OF YOUR CODE                            #
	###########################################################################
	return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
	...
	###########################################################################
	N, C, H, W = x.shape
	# Transpose x to shape N*H*W*C
	out,cache = batchnorm_forward(np.transpose(x,(0,2,3,1)).reshape((N*H*W, C)), gamma,beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
	...
	###########################################################################
	N, C, H, W = dout.shape
	# Transpose dout to shape N*H*W*C
	dx, dgamma, dbeta = batchnorm_backward(np.transpose(dout,(0,2,3,1)).reshape((N*H*W, C)), cache)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
	...
	out = np.zeros_like(x)
	cache = []
	lst = np.split(x, G, axis=1)
	width = int(C/G)
	for g in range(G):
		re = np.transpose(lst[g],(0,2,3,1)).reshape((N*H*W, width))
		out2,cache2 = layernorm_forward(re, gamma[g*width:(g+1)*width], beta[g*width:(g+1)*width], gn_param)
		out2 = np.transpose(out2.reshape((N,H,W,width)), (0,3,1,2))
		out[:,g*width:(g+1)*width,:,:] = out2
		cache.append(cache2)
	cache.append(G)
	###########################################################################
	#                             END OF YOUR CODE                            #
...
def spatial_groupnorm_backward(dout, cache):
	...
	dgamma = np.zeros(C)
	dbeta = np.zeros(C)
	lst = np.split(dout, G, axis=1)
	for g in range(G):
		re = np.transpose(lst[g],(0,2,3,1)).reshape((N*H*W, width))
		dx2,dgamma2,dbeta2 = layernorm_backward(re, cache[g])
		dx2 = np.transpose(dx2.reshape((N,H,W,width)), (0,3,1,2))
		dx[:,g*width:(g+1)*width,:,:] = dx2
		dgamma[g*width:(g+1)*width] += dgamma2
		dbeta[g*width:(g+1)*width] += dbeta2
